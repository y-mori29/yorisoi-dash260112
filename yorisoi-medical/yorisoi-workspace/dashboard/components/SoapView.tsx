    ...
    <div className="flex flex-col gap-6 pb-20">
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <EditableSoapBlock letter="S" title="Subjective (服薬状況・効果・副作用・困りごと)" value={soap.s} onChange={(v) => updateSoap('s', v)} color="bg-sky-400" />
          <EditableSoapBlock letter="O" title="Objective (残薬・バイタル・他職種情報)" value={soap.o} onChange={(v) => updateSoap('o', v)} color="bg-rose-400" />
          <EditableSoapBlock letter="A" title="Assessment (薬学的評価・処方監査)" value={soap.a} onChange={(v) => updateSoap('a', v)} color="bg-amber-400" />
          <EditableSoapBlock letter="P" title="Plan (指導内容・医師への提案・次回確認)" value={soap.p} onChange={(v) => updateSoap('p', v)} color="bg-emerald-400" />
        </div>
      </Card>
