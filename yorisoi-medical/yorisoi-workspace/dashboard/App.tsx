      ...
      <div className={`flex-1 flex flex-col h-full relative bg-gray-50 ${!selectedPatientId && !selectedRoundId ? 'hidden md:flex' : 'flex'}`}>
        ...
        <header className="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-4 md:px-6 shrink-0 z-10 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Cloud Sync Button (E2E Demo) */}
            <button
              onClick={async () => {
                showToast('クラウドから最新の録音を取得中...', 'info');
                try {
                  const { api } = await import('./services/api');
                  const jobs = await api.fetchRecentJobs();
                  if (jobs.length === 0) {
                    showToast('新しい録音は見つかりませんでした', 'info');
                    return;
                  }

                  // Fetch details for each job
                  const newRecords = await Promise.all(jobs.map(async (job) => {
                    const detail = await api.fetchJob(job.jobId);
                    if (!detail) return null;

                    const summary = (detail as any).summary || {};
                    const soap = summary.soap || { s: '', o: '', a: '', p: '' };
                    const report100 = summary.report_100 || '';

                    return {
                      id: `rec_${job.jobId}`,
                      date: new Date().toISOString().split('T')[0],
                      transcript: detail.transcript || '（文字起こし中...）',
                      clinicalData: {
                        soap: soap,
                        report_100: report100,
                        summary: report100,
                        home_visit: { basic_info: '', chief_complaint: '', observation_treatment: '', medication_instruction: '', next_plan_handover: '' },
                        pharmacy_focus: { medications: [], adherence: '', side_effects: [], drug_related_problems: [], labs_and_monitoring: [], patient_education: [], follow_up: '' },
                        alerts: { red_flags: [], need_to_contact_physician: [] },
                        meta: { main_problems: [], note_for_pharmacy: 'クラウド同期データ' }
                      },
                      status: 'pending' as const
                    };
                  }));

                  // Update State grouped by patient
                  setPatients(prevPatients => {
                    ...
                    jobs.forEach((job, index) => {
                      ...
                      const targetPatientId = job.patientId || 'p1_demo';

                      updatedPatients = updatedPatients.map(p => {
                        if (p.id !== targetPatientId) return p;
                        if (p.records.some(r => r.id === record.id)) return p;
                        return {
                          ...p,
                  ...
                  showToast(`${jobs.length}件の録音を同期しました`, 'success');

                } catch (e) {
                  showToast('同期に失敗しました', 'info');
                  console.error(e);
                }
              }}
              className="flex items-center gap-1 px-3 py-1.5 text-sm font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors border border-blue-200"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
              クラウド同期
            </button>
