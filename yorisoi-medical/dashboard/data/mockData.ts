...
const dataScenario1: ClinicalData = {
  soap: {
    subjective: "疼痛コントロール良好（NRS 2/10）。オキシコンチン有効。便秘（排便3日に1回、腹満感あり）訴えあり。眠気・ふらつきは生活に支障なし。",
    objective: "残薬確認：マグミット（3シート・未服用）、オキノーム（前回処方分残あり・今回使用なし）。バイタル安定。",
    assessment: "オキシコンチンによる便秘と思われる。マグミット服用に対する抵抗感（多剤懸念）あるが、疼痛管理継続のため排便コントロール必要と判断。オキノームは疼痛安定しており使用頻度低下。",
    plan: "マグミット服用の重要性説明し、服用勧奨（まずは1日1回から）。オキノームは今回処方なしで調整。次回、排便状況と眠気・ふらつきの再確認。"
  },
...
const dataScenario2: ClinicalData = {
  soap: {
    subjective: "朝の服薬は良好だが、夜間は生活動作の都合で飲み忘れ頻発（週3-4回）。立ちくらみ（めまい）の訴えあり。",
    objective: "残薬確認：夕食後薬約7日分残。血圧手帳未確認だが、自覚症状（立ちくらみ）あり。",
    assessment: "夕食後の服用時点が生活リズムと合っていない可能性。アドヒアランス低下により血圧変動・症状発現のリスク。服用タイミングの変更（歯磨き時など）が有効と考える。",
    plan: "夕食後薬の服用タイミングを「夕食後」から「就寝前（歯磨き時）」等の忘れにくい時間帯へ変更提案。服薬カレンダーや設置場所の工夫指導。めまいの経過観察。",
  },
...
const dataScenario3: ClinicalData = {
  soap: {
    subjective: "抗がん剤開始後、悪心・食欲不振あり。ドンペリドン（制吐剤）は「副作用が怖い」との理由で未使用。",
    objective: "残薬確認：ドンペリドン全量（10回分）残。食事摂取量低下。",
    assessment: "悪心によるQOL低下・栄養摂取不足のリスク。制吐剤に対する不安感（ノセボ効果懸念）があるが、利益（食欲回復）が上回ることを説明し使用を促す必要あり。",
    plan: "ドンペリドンの効果と安全性を再説明し、まずは1回試用するよう指導。副作用（しびれ等）は現時点なし。次回、使用状況と悪心の改善度確認。"
  },
...
const dataScenario4: ClinicalData = {
  soap: {
    subjective: "変化なし。元気である。",
    objective: "定期配送のみ。玄関先対応。顔色良好。残薬なし（本人申告）。",
    assessment: "容体安定。アドヒアランス良好と判断。",
    plan: "定期薬継続。次回も定期訪問。"
  },
...
const dataScenario5: ClinicalData = {
  soap: {
    subjective: "寒くなり血圧高め。医師よりアムロジピン増量（5mg→10mg）の指示あり了承。足のむくみについて医師より説明受けている。",
    objective: "医師診察同席。アムロジピン5mg残薬なし（飲みきり）。新規10mg処方開始。",
    assessment: "増量に伴う降圧効果と副作用（浮腫）のモニタリングが必要。旧用量の残薬がないため切り替えはスムーズに行える。",
    plan: "新規用量（10mg）での開始確認。下肢浮腫の初期症状について具体的（靴下の跡等）に説明。次回、血圧値と浮腫の有無を確認。"
  },
